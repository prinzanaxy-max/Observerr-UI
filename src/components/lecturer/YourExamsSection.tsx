...
const YourExamsSection = memo(({ exams, activeTab, loading = false, onTabChange }: YourExamsSectionProps) => {
  const navigate = useNavigate();

  const handleExamAction = (exam: LecturerExam) => {
    if (exam.status === 'live') {
  ...
  return (
    <section>
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
        ...
        <Link to="/lecturer/exams" className="text-student-primary font-student font-bold text-student-body-md hover:underline text-left">
          See all
        </Link>
      </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 animate-pulse">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="h-48 bg-student-surface-container-high rounded-brand" />
          ))}
        </div>
      ) : exams.length === 0 ? (
