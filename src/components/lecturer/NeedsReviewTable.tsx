...
const NeedsReviewTable = memo(({ students, loading = false, onViewTimeline }: NeedsReviewTableProps) => (
  <section className="bg-student-surface-container-lowest p-4 sm:p-6 rounded-brand shadow-[0px_10px_30px_rgba(0,0,0,0.05)] border border-student-outline-variant/30">
    <div className="flex justify-between items-center mb-6">
      ...
      <Link to="/lecturer/students" className="text-student-primary font-student font-bold text-student-body-md hover:underline">
        See all
      </Link>
    </div>
    ...
    {loading ? (
      <div className="space-y-4 animate-pulse">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="h-12 bg-student-surface-container-high rounded-xl" />
        ))}
      </div>
    ) : students.length === 0 ? (
      <p className="py-8 text-center text-student-on-surface-variant font-student text-student-body-md">
        No sessions need review right now.
      </p>
    ) : (
              ...
              <tr key={student.id} className="hover:bg-student-surface-container-low transition-colors group">
                ...
                <td className="py-4 text-right">
                  <button
                    ...
                    disabled={!student.latestSessionId}
                    className="bg-student-surface-container-high hover:bg-student-primary-container text-student-on-surface-variant hover:text-student-on-primary-container px-4 py-2 rounded-xl text-student-body-md font-student font-bold transition-all disabled:opacity-50"
                  >
                    View Timeline
                  </button>
                </td>
