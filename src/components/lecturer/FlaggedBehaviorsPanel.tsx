...
const FlaggedBehaviorsPanel = memo(({ behaviors, loading = false }: FlaggedBehaviorsPanelProps) => (
    ...
    {loading ? (
      <div className="space-y-6 animate-pulse">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="h-10 bg-student-surface-container-high rounded-full" />
        ))}
      </div>
    ) : behaviors.length === 0 ? (
      <p className="text-student-on-surface-variant font-student text-student-body-md py-4">No flagged behaviors in this period.</p>
    ) : (
